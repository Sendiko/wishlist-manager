    ...
    return (
        <form action={action} className="space-y-6">
            ...
            <div>
                ...
                {state?.errors?.categoryId && <p className="mt-1 text-sm text-red-600">{state.errors.categoryId}</p>}
            </div>

            {/* Photo Upload */}
            <div className="bg-surface-container-low p-4 rounded-md border border-outline-variant">
                <label htmlFor="photo" className="block text-sm font-medium text-on-surface-variant mb-2">Upload Foto</label>
                <input
                    type="file"
                    id="photo"
                    name="photo"
        ...
        hover:file:bg-primary-container/80"
                />
                <p className="mt-2 text-xs text-on-surface-variant">Atau masukkan URL foto di bawah:</p>
            </div>
