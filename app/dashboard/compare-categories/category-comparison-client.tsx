                    ...
                    <div className="flex gap-6 min-w-max">
                        {categoriesWithStats.map((category) => {
                            ...
                            return (
                                <div
                                    key={category.id}
                                    className={`w-80 flex-shrink-0 bg-surface rounded-xl shadow-md border-2 overflow-hidden flex flex-col relative transition-all duration-300 ${isWinner ? 'border-primary shadow-xl scale-105 z-10' : 'border-outline-variant opacity-90'}`}
                                >
                                    {isWinner && (
                                        <div className="absolute top-0 inset-x-0 bg-primary text-on-primary text-center py-1 text-xs font-bold uppercase tracking-wider z-20">
                                            Winner 💎
                                        </div>
                                    )}
                                    ...
                                    <div className={`p-6 flex-grow flex flex-col space-y-6 ${isWinner ? 'pt-10' : ''}`}>
                                        ...
                                        <div className="space-y-4">
                                            {/* Total Score */}
                                            <div className="bg-primary-container p-4 rounded-lg text-center">
                                                <p className="text-xs text-on-primary-container font-bold uppercase tracking-wider">Total Value Points</p>
                                                <p className="text-3xl font-extrabold text-on-primary-container">{category.totalScore}</p>
                                            </div>

                                            {/* Stats Grid */}
                                            <div className="grid grid-cols-2 gap-3">
                                                <div className="bg-surface-container p-3 rounded-lg text-center">
                                                    <p className="text-xs text-on-surface-variant uppercase">Total Harga</p>
                                                    <p className="font-bold text-on-surface text-sm">{formatCompactCurrency(category.totalPrice)}</p>
                                                </div>
                                                <div className="bg-surface-container p-3 rounded-lg text-center">
                                                    <p className="text-xs text-on-surface-variant uppercase">Rata-rata</p>
                                                    <p className="font-bold text-on-surface text-sm">{category.avgScore}</p>
                                                </div>
                                            </div>

                                            {/* Selected Items List */}
                                            <div className="mt-4 pt-4 border-t border-outline-variant">
                                                <p className="text-xs text-on-surface-variant font-semibold mb-2 uppercase">Item Terpilih:</p>
                                                <ul className="text-xs text-on-surface space-y-1 max-h-32 overflow-y-auto">
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {categories.map((category) => {
                    ...
                    return (
                        <div key={category.id} className={`bg-surface rounded-xl shadow-sm border transition-all duration-200 ${selectedCount > 0 ? 'border-primary shadow-md' : 'border-outline-variant'}`}>
                            {/* Category Header */}
                            <div className={`p-4 border-b flex justify-between items-center ${selectedCount > 0 ? 'bg-primary-container' : 'bg-surface-container'}`}>
                                <h3 className={`font-bold flex items-center gap-2 ${selectedCount > 0 ? 'text-on-primary-container' : 'text-on-surface'}`}>
                                    <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${selectedCount > 0 ? 'bg-primary text-on-primary' : 'bg-surface-container-high text-on-surface-variant'}`}>
                                ...
                                {category.items.map(item => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`flex items-center p-2 rounded-lg cursor-pointer transition-colors ${isSelected ? 'bg-primary-container/20' : 'hover:bg-surface-container-high'}`}
                                        >
                                            <input
                                                type="checkbox"
                                                checked={isSelected}
                                                onChange={() => { }} // Handled by div click
                                                className="rounded text-primary focus:ring-primary border-outline mr-3"
                                            />
                                            <div className="flex-grow min-w-0">
                                                <p className={`text-sm font-medium truncate ${isSelected ? 'text-primary' : 'text-on-surface'}`}>{item.name}</p>
                                                <p className="text-xs text-on-surface-variant">{formatCompactCurrency(item.price)} • Score: {item.wish_rate + item.neccessary_rate + item.interest_rate}</p>
                                            </div>
                                        </div>
                                    )
                                })}
            ...
            {categories.every(c => c.items.length === 0) && (
                <div className="text-center py-12 text-on-surface-variant bg-surface rounded-lg border border-dashed border-outline-variant">
